...
| 달력 | [cal.jsx](src/cal.jsx) | 빈 시간 계산, 약속 요청 |
...
월 그리드는 `now`가 읽는 데이터를 그대로 씁니다. 약속 잡기에서 제안하는 빈 시간도 **같은 데이터에서 계산**하므로, 제안된 시간은 실제로 비어 있는 시간입니다. 평일 10시부터 18시, 30분 단위, 지난 시간과 이미 찬 시간을 뺍니다.

**확정 예약은 여기서 못 합니다.** 브라우저 키는 읽기 전용이고, 이벤트를 만들려면 방문자가 아니라 소유자의 OAuth가 필요합니다. `calendar.app.google`은 `X-Frame-Options: SAMEORIGIN`이라 프레임에도 못 넣습니다.

그래서 **이미 있는 통로**를 씁니다. 요청은 채팅 파이프로 나가서 휴대폰에 도착하고, 답장은 채팅 창으로 돌아옵니다. 창은 이게 요청이지 확정이 아니라고 말합니다. Crisp가 차단돼 있으면 그렇다고 말하고 같은 내용을 담은 `mailto:` 를 줍니다. 방문자 자신의 캘린더에 가예약으로 넣는 링크도 함께 줍니다. 그건 이 페이지가 건드릴 수 있는 유일한 캘린더입니다.
...
데이터가 아니라 **파일 자체**를 넣고 싶으면 [public/home/](public/home/) 에 놓습니다. 이 폴더는 가상 홈(`/home/jeongin`)의 거울입니다. 디스크의 트리가 그대로 파일시스템의 트리가 됩니다.

```
public/home/papers/kcc2026.pdf   ->  ~/papers/kcc2026.pdf
public/home/Desktop/메모.md      ->  ~/Desktop/메모.md
```

`data.js` 를 고칠 필요도, 어딘가에 등록할 필요도 없습니다. 빌드가 폴더를 훑어 목록을 번들에 주입하고, `public/` 전체가 `dist/` 로 복사되므로 **같은 오리진**에서 서빙됩니다. PDF 뷰어가 CORS나 프레이밍을 협상할 일이 없어 가장 확실하게 열리는 경로입니다.
...
|---|---|
| `.pdf` | PDF 뷰어로 열립니다 |
| 텍스트 (`.md` `.txt` `.json` `.csv` 등) | 내용을 빌드 시점에 인라인해서 `cat` 과 뷰어가 읽습니다. 256KB까지 |
| 그 외 | 링크 노드가 되어 브라우저로 넘깁니다. 렌더하지 못하는 걸 렌더하는 척하지 않습니다 |

이미 있는 디렉터리에 넣으면 **합쳐집니다.** `public/home/Desktop/` 에 파일을 놓아도 기존 README와 런처는 그대로 있습니다. 같은 이름이면 넣은 파일이 이깁니다. 폴더에 파일을 놓는 건 의도적인 행동이니까요.

점으로 시작하는 이름(`.gitkeep` 등)은 무시합니다.
